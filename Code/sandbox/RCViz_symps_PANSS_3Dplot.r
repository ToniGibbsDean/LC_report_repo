...
plot_ly(threeDdat, x = ~gTotal, y = ~nTotal, z = ~pTotal, 
                    marker = list(color = ~total, sizemode = 'diameter', colorscale = c('#FFE1A1', '#683531'), sizes = c(5, 6), showscale = TRUE))
...
threeDdat <- dat %>%
  ...
  as_tibble %>%
  group_by(person=as.factor(person)) %>%
  summarise(gafDif=gaf_12_months-current_gaf, pTotal = rowSums(across(starts_with("p"))/42), nTotal = rowSums(across(starts_with("n"))/42), gTotal = rowSums(across(starts_with("g")))/96) %>%
  #summarise(pTotal=sum(p1:p7), nTotal=sum(n1:n7), gTotal=sum(g1:g16)) %>%
  #group_by(person) %>%
  mutate(total=(pTotal/42+nTotal/42+gTotal/96))

plot_ly(threeDdat, x = ~gTotal, y = ~nTotal, z = ~pTotal, 
                    marker = list(color = ~gafDif, sizemode = 'diameter', colorscale = c('#FFE1A1', '#683531'), sizes = c(5, 6), showscale = TRUE))

plot_ly(threeDdat, x = ~gTotal, y = ~nTotal, z = ~pTotal, size =~gafDif,
                    marker = list(color = ~total, sizemode = 'diameter', colorscale = c('#FFE1A1', '#683531'), sizes = c(5, 6), showscale = TRUE))
