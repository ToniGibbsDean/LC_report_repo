...
myColours1=c("1"=depn_insti, "2"=col_dislike_alot, "3"=col_dislike, "4"=col_neutral,"5"= col_like,"STEP"=col_like_alot,"All"= "#000000")
...
table1<- knitr::kable(inq_tabxRegion) #,format="markdown")
...
dat_weeklyqueries[nrow(dat_weeklyqueries)+1,]<-list(date("2024-01-26 UTC"), 0, 0, "All")

RegionsXY <- x %>%
        group_by(REGION) %>%
        summarize(pos = which.max(Inqweek),
                    x = Inqweek[pos],
                    y = cumsum[pos],
                    label = REGION[pos])
...
inquiryRunchart<-dat_weeklyqueries_region %>% 
    ...
    ggplot() +
        ...
        geom_line(data=dat_weeklyqueries, aes(x=Inqweek, y=cumsum, colour=REGION)) +
        theme_classic() +
        ...
        scale_color_manual(name="heardAboutInititative", values=myColours1) +
        geom_label_repel(data=xy_labs, aes(x=x, y=y+1, label = label), 
                        label.size = 0.05,
                        nudge_x = 0.01, 
                        inherit.aes=F, 
                        na.rm = FALSE) 