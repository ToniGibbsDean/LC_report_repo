    ...
    dat <- read_excel("Data/LHN & STEP - PhoneScreen and CONSORT Data V2.xlsx", skip = 1)  %>%
        ...
        tabLMHArefs_consent_new <- dat %>%
                    ...
                    mutate(`Admitted to`=recode(`Admitted to`,
                                                    "Cornell Scott Hill Health Center"="CSHC")) %>%
                    mutate(`Admitted to`=as.factor(`Admitted to`)) %>%
                    rename(`Referred to` = LMHA) %>% 
                    mutate(`Referred to`=recode(`Referred to`,
                                                    "Western CT mental health network - waterbury area"="Western CT",
                    ...
                    select(`Referred to`, REGION, FU_admitted_numeric, waitTime_inq2admit, `Admitted to`) %>%
                    group_by(`Referred to`, `Admitted to`) %>%
                    summarise(`No. admitted`=sum(FU_admitted_numeric), 
                              `Median  wait time (days)`=median(as.numeric(waitTime_inq2admit)), 
                              `IQR` = IQR(as.numeric(waitTime_inq2admit))) %>%
                    mutate_if(is.numeric, round, 1) %>%
                    mutate(`Admitted to`=replace_na(as.character(`Admitted to`), "Matches referral"))
        
DUPTable <- knitr::kable((tabLMHArefs_consent_new), format="markdown")